<script lang="ts">
    ...
    $effect(() => {
        ...
        if (typeof sessionStorage !== 'undefined') {
            dismissed =
                sessionStorage.getItem('news-portal-breaking-dismissed') === 'true';
        }
    });
...
{#if !dismissed && breakingNews.length > 0}
    ...
    >
        <div class="mx-auto flex max-w-7xl items-center gap-3 px-4 py-2 lg:px-6">
            <div class="shrink-0 rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-[0.28em] text-white">
                🔴 Срочно
            </div>
