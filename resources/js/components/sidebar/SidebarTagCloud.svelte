<script lang="ts">
    ...
    import { filters, toggleTag } from '@/stores/articles.svelte.js';
    ...
    const tags = $derived((appState.trendingTags ?? []) as Tag[]);
    ...
    <div class="flex flex-wrap gap-2">
        {#each tags as tag (tag.id)}
            ...
            >
                ...
                {#if active}
                    <span class="rounded-full bg-white/20 px-1.5 py-0.5 text-[0.65rem]">
                        {tag.usage_count ?? 0}
                    </span>
                {/if}
            </button>
