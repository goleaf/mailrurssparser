<script lang="ts">
    ...
    import { filterBarArticleContentTypeOptions } from '@/features/articles/data/articleEnums';
    import {
        activeFiltersCount,
        clearDate,
        filters,
...
<section
    class="rounded-[2rem] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(248,250,252,0.9))] p-5 shadow-[0_25px_80px_-60px_rgba(15,23,42,0.55)] dark:border-white/10 dark:bg-[linear-gradient(180deg,rgba(15,23,42,0.92),rgba(15,23,42,0.82))]"
>
    ...
    {#if showAdvanced}
        ...
        >
            ...
            <div>
                <div
                    class="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-slate-400"
                >
                    <CalendarDays class="size-4" />
                    Период
                </div>
                <div class="mt-3 flex flex-wrap gap-2">
                    {#each [{ key: 'today', label: 'Сегодня' }, { key: 'yesterday', label: 'Вчера' }, { key: 'week', label: 'Эта неделя' }, { key: 'month', label: 'Этот месяц' }, { key: 'custom', label: 'Произвольный' }] as preset (preset.key)}
                        <button
                            type="button"
                            class={cn(
                                'rounded-full border px-3 py-2 text-sm transition',
    ...
    <div
        class="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-[1.5rem] border border-slate-200/70 bg-slate-50/80 px-4 py-3 dark:border-white/10 dark:bg-white/5"
    >
        <div class="text-sm text-slate-500 dark:text-slate-400">
            Найдено
